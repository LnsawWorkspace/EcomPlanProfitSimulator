...
> **版本记录：** `version.md`（功能迭代后更新版本历史与当前版本号）。

## 这个 Skill 是做什么的？

> **一句话：帮你算清楚"卖一件商品到底能赚多少钱"（理论/理想情况下的单品利润）。**

**适用人群**：国内电商（拼多多/京东/淘宝/抖音）、跨境电商、外卖商家……只要你是**卖货的**，想知道"这个品按这个价卖、这个推广投入、这个退款率、这些乱七八糟的支出，到底赚不赚、赚多少"，就能用。

**它计算的是"理想/理论情况"**：按你填的售价、成本、推广 ROI、退款率等多种自定义参数，模拟算出一件单品的利润、利润率、资本回报率、保本点——**帮你做决策**（定价多少、推广投多少、要不要提价、能承受多大退款），不是平台后台的真实流水。

**怎么用（很简单）**：
- 直接问 AI：**"这个 全场景单品利润测算Skill 能帮我做什么？"** —— AI 会告诉你能做的事，并带你走通完整流程（建方案 → 填参数 → 看报告 → 调优）。
- 或者直接说你的需求，比如：*"帮我算一下这个品售价 99、成本 68、退款率 8%，利润多少？"* —— AI 会帮你跑。
- **建议：** 尽可能的提供完整参数，AI并不知道你面临的情况。只有当参数越具体、越多、越准确，AI 才能算出越准确的利润结果。
---
...
+ **⚠ 运行模式要求：本技能必须通过操作浏览器（Playwright 驱动）才能执行，因此 AI 必须处于「可执行/操作浏览器」的模式下（如 Craft 模式），不能是 Plan / Ask 模式**——Plan/Ask 下 AI 只能阅读、分析、出方案，无法真正驱动浏览器干活。**推荐用法：先用 Plan/Ask 模式和 AI 沟通需求、确认方案，再切换到可执行模式（Craft）实际运行。**（Plan/Ask 模式下 AI 仍可阅读并理解本技能的全部文档，只是不执行浏览器操作。）
+ **⚠ 浏览器运行模式要求：本技能必须运行在正常的浏览器（有头模式）下——浏览器窗口可见、操作过程能被人看到。禁止无头模式（headless）；禁止无痕/隐私窗口、隔离/临时 profile（每次新建的临时浏览器环境）。原因：网站的全部数据（数据空间/方案组/方案/参数）都存在浏览器 IndexedDB 里，绑定固定的持久 profile——用无头/无痕/隔离/临时环境操作，数据保存不下来（关闭即丢、下次看不到），等于白干。**

## 外部依赖
**使用前请先检查外部依赖。**
+ **浏览器**：Edge（Chromium 内核）或 Chrome，需开启远程调试端口（9222），并常驻后台运行。（常驻运行的目的是减少多余的打开关闭动作。）（不需要用户自行设置，AI会处理。）**必须以有头模式（正常窗口）运行，并使用固定的持久 profile（user-data-dir）——站点数据存于 IndexedDB、绑定 profile，无头/无痕/隔离/临时 profile 下数据保存不下来，禁止使用。**
...
4. **方案调优（闭环，完整流程）**：**先读原始报告**（planReport.md §8 分析套路：读结构→对口径→找病灶）→ **再看相关敏感性分析图**（graphs.md 各图：折线看保本点、热力图看组合空间，别只看报告就调）→ 综合定病灶 → `plan_params_ops.js set` 改参 → `read` 再验证。例：把"拼多多推广"资本回报率从 −14% 调到 +31%（先看报告定位广告费占比 47%，再看 ROI 图/售价图确认保本点，然后改售价 139.9 / 退款 8% / ROI 4 验证）。

**使用前提**：浏览器常驻模式（脚本自动启动带 CDP 9222 的 Edge 并复用）；**必须有头模式 + 固定持久 profile（禁无头/无痕/隔离，见上方「重要提示」）**；用托管 node 运行（路径见脚本头注释/运行环境）。
...
+ **同渠道比价用售价图（使用技巧）**：想对比同一渠道下不同售价（其他参数不变）的利润变化，直接看售价敏感性分析图（`sale`）即可，不必为每个售价建多个方案。
...
- **用户在使用本技能/网站时遇到任何问题，可以在 [skillhub.cn](https://skillhub.cn) 搜索「全场景单品利润测算」找到本 Skill，写评论反馈**——改进方向依赖真实用户反馈。
- **如果觉得好用/有用，也请给个评价**——让这个 Skill 真的有人在用（这是它持续变好的最大动力）。
- **用户不想自己写？可以让我（AI）代写评论**：告诉我想反馈/评价的内容，AI 会整理成评论草稿给用户确认后发布。
- **AI 记住**：用户提出反馈/评价需求时，主动引导到 skillhub.cn 的「全场景单品利润测算」页面；代写评论时忠实转述用户原意，不夸大不虚构。
