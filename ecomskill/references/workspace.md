...
> 本文所有结论均来自 2026-08-20 对线上源码的逐行核对 + 真机全链路实测（create→rename→activate→delete→doctor 全部跑通）。
> 配套脚本：`scripts/workspace_ops.js`（站点自身没有体检功能，只能靠它）。
...
> - 维护时把它当**只读目录**看待：脚本只读取它的 `workspaces` 记录来枚举/定位工作区；**绝不**把它当作一个工作区去 list / delete。
...
| 列表 | `#workspace-table-body tr` | 列顺序：名称、描述、更新时间、备份时间（恒显示 `-`，无需关注）、操作 |
...
"$N" workspace_ops.js delete "空间名|ID" --confirm   # 高危：删除不可恢复
"$N" workspace_ops.js repair --fix multi-enabled,clear-deleting --confirm
```

所有命令最后一行输出 `RESULT: {json}`，程序化解析取这一行即可；加 `--json` 只留这一行。

### 4.1 数据备份（用户自行负责，脚本不再提供）

**已明确放弃自动备份能力**：站点数据 100% 存在浏览器 IndexedDB 里，脚本不再提供 export/import 备份恢复功能。

- 站点面板底部有"请定期备份数据"的提醒，但**没有**任何备份/导出按钮；备份只能由**用户自己**完成：找到本浏览器 profile 的 IndexedDB 目录，手动复制对应站点（ecomplanprofitsimulator.lnsaw.com）的文件夹。
- `delete` 是高危操作，删除前请自行确认数据不再需要（站点无备份可恢复）。
- 换浏览器/换设备 = 数据无法迁移（无后端、无账号、无云同步）；清 cookie 会连带清 IndexedDB = 数据全丢。
...
| `orphan-plan` / `orphan-params` | warn | 方案指向已删除的组 / 参数没有对应方案 | 数据残渣，不影响使用；洁癖可手动清理 |
| `persistence-off` | warn | 存储未持久化，磁盘紧张时浏览器可能自行清理 IndexedDB | 多用几次站点提升 engagement，或 `repair --fix request-persist --confirm` |
| `quota-high` | warn | 用量 > 80% 配额（实测配额约 10 GB） | 清理无用空间 |
...
1. **`Repository_Workspace.getAllWorkspaces()` 的 `return` 写在 for 循环体里**
   ...
   → 同一处 `map` 只挑了 6 个字段，**丢掉了 `deleteing`**。连带后果：`deleteing` 的"下次打开自动重删"自愈逻辑**永远不会触发**（读回来恒为 undefined）。
...
3. **当前启用的空间删不掉**：`confirmDelete` 直接拦截并 toast「无法删除当前启用的工作区…」。先 `activate` 另一个再删。

4. **激活后页面会 `location.reload()`**（作者原话：省得写 destroy 手工 GC）。自动化点完「激活」必须等导航结束再断言，否则读到的是旧 DOM。

5. **名称全局唯一**，重名保存报 danger toast「工作区名称已存在」。批量脚本要先查重再建。

6. **删除是双重确认**：第一个确认框只是跳板，真正执行在 `#remove-workspace-confirm-btn-agin`。少点一步会以为"删了但没删"。

7. **持久化默认可能未启用**：`navigator.storage.persist()` 只在 engagement 足够时才被批准（实测首次 `未启用`，多次打开面板后变 `已启用`）。未启用状态下磁盘紧张时浏览器可清理 IndexedDB —— 这是真实的数据丢失路径，`doctor` 会告警。

8. **`indexedDB.open(不存在的库名)` 会顺手创建一个空库**——脚本里所有读操作都先用 `indexedDB.databases()` 判存在，避免制造孤儿空库。自己写临时脚本时最容易在这里污染环境。

9. 小瑕疵（无功能影响，别误报为故障）：`workbench.js` 绑定了 HTML 里并不存在的 `removeSpaceBtn`；二次确认弹窗静态文案有错字"高微操作"（运行时被 JS 覆盖成"高危操作"）；表格里有一行 `ws_001` 静态占位行，渲染时会被清掉。
...
- **不要** 把 `profitSimulation_systemDB`（系统目录库）当成"又一个工作区"去列/删——它是内部目录库（类比 SQL Server 的 master），只应由站点自身读写；脚本已通过 `isSystemOwnedDb()` 将它排除在用户库/孤儿库枚举之外，永不把它当工作区删除。
