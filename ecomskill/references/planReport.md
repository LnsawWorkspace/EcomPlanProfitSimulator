...
- **调优完整流程（用户确立，2026-08-20）**：**先看原始报告（本文档 §8）→ 再看 6 个敏感性分析图（graphs.md：ROI/售价/单量折线看保本点，热力图看组合空间）→ 综合定病灶 → `plan_params_ops.js set` 改参 → `read` 验证**。光看报告就调，容易漏掉"保本点/安全边际/组合空间"这些图才看得见的信息。
