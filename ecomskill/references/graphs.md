...
- **图例（legend）交互（单变量折线图）**：图上方有图例，可点切换**显示/隐藏曲线**——等价于"人类选中要看的曲线"。**页面默认只显示「利润 + 利润增长率」两条**（其余初始隐藏），AI 读图默认拿全量 series；截图/聚焦时用 `--show-series "利润,资本回报率"` 只显示指定曲线（脚本用 `setOption({legend:{selected}})` 一次性设置，避开页面 legendselectchanged 监听的时序干扰）。
  **⚠ 量程压制（用户 2026-08-20 明确）**：多图例同时显示时，**大量程曲线会把小量程曲线压成一条直线**——y 轴刻度由大数值曲线（利润/广告费，几万）主导，小数值曲线（利润率 0.2、增长率 0.01、回报率）就被压扁到贴底/贴顶的直线，看不出形状。**所以聚焦看某条小量程曲线时，务必用 `--show-series "利润率"` 只看它**，避免被其他图例影响（读数据不受影响——series 全量照读，只是显示/截图聚焦）。
