...
self.onmessage = function (e) {
    ...
    for (let salePoints = saleStart; salePoints.lte(saleEnd); salePoints = salePoints.plus(saleStep)) {
        ...
        for (let volumePoints = new Decimal(e.data.volumeStart); volumePoints.lte(new Decimal(e.data.volumeEnd)); volumePoints = volumePoints.plus(new Decimal(e.data.volumeStep))) {
