...
class PlanReportSaleGraphManager {
    ...
    #showReport() {
        ...
        // 直接修改销量，不用传递了
        this.#planParams.modelPlanParamsSale.payOrderQuantity = new Decimal(document.getElementById('volume-graph').value);
        ...
        worker.postMessage({
            ...
            roiStart: RoiStart.toString(),
            roiEnd: RoiEnd.toString(),
            roiStep: RoiStep.toString(),
        ...
        worker.onmessage = (e) => {
            ...
            const chartData = results.map(planReport => ([
                planReport.planParams.modelPlanParamsAdvertising.roi.toString(),
            ...
            let option = {
                tooltip: {
                    ...
                    formatter: function (params) {
                        return `售价：${params.value[1]} - roi：${params.value[0]} : 利润：${params.value[2]}`;
                    }
                },
                xAxis: {
                    type: 'category',
